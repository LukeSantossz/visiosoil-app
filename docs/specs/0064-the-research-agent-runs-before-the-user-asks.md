...
- **Assumption:** guidance keyed by class, clay-activity family, unit, biome and land use is specific enough to be useful. The feedback loop in the observability section is what would falsify it.
...
- **Risk:** the twice-yearly rebuild cadence outlives its funding. At roughly $28 a year against a reserve of at most $34 that Tier 2 also draws from, year one is funded and year two is not. Recorded in the architecture document rather than mitigated, because the mitigation is recurring budget and that is not this document's to grant.
