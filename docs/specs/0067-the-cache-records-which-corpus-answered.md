...
wart SPEC 0066 left on purpose: a row written while the app held no corpus has
...
- `upsert_writes_null_when_the_result_has_no_version`: the absent-corpus result
  SPEC 0066 produces writes null rather than a placeholder.
...
- **Risk:** the migration runs on a device holding rows written by SPEC 0066's
