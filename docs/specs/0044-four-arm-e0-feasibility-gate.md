...
- **The execution hardware is an open input, and the gate may run on a GPU.** *Amended 2026-09-05, at the Developer's direction.* This spec was written assuming no local GPU. That premise is **withdrawn rather than replaced**: the device is chosen when the gate is scheduled, and each choice carries consequences that are cheaper to know now than ten hours into a run.

  - **What it moves.** The roughly 930 s per fold these estimates rest on is a CPU figure for the incumbent, so a GPU moves `cnn` and `shuffled_control` materially and the twenty-hour total with them. It moves the descriptor arm not at all: that arm is scikit-learn over 26 features and is CPU-bound whatever the host.
  - **An arm cannot straddle two devices.** `train.runtime_mode` records `device` and `gpu_count` per fold, and [SPEC 0056](0056-an-interrupted-arm-resumes-instead-of-starting-over.md)'s `require_uniform_runtime` refuses an arm whose folds did not all record one runtime. So the device is settled per arm **before that arm's first fold**, and a CPU-started arm cannot be resumed on a GPU: the resume is refused, correctly, and `--force` is the only way through — which discards the CPU folds. That refusal is what the record is for and a trap if it is met late.
  - **Operator determinism must stay on, and a GPU is the reason it exists.** `training.deterministic_ops` defaults true because seeding alone does not make TensorFlow's kernels deterministic: several reduce across threads in completion order, so float addition happens in a different order each run. On CPU this does not arise; on GPU it does. A run that trades the flag for throughput is not comparable with one that did not, and `runtime.json` records it so that is detectable rather than silent.
  - **Arms on different devices stay comparable.** Every contrast is on group-level correctness and none is on wall clock, so the incumbent on a GPU against the descriptor arm on a CPU compares methods and not hardware. What would contaminate a contrast is the **determinism flag** differing between two arms, which is the point above and not the device itself.
  - **A hosted GPU moves the archive.** `ml/data/datasets/**` is a build product and is not tracked ([ADR 0019](../adr/0019-a-dataset-version-is-a-build-product-and-nothing-under-it-is-versioned.md)), so running anywhere but this machine means copying 221 photographs of a laboratory's samples to it. That is a data-handling decision and not a scheduling one; it is named here so it is taken deliberately rather than discovered at upload time.
  - **Condition 3 is untouched.** It is inference latency on a phone. The hardware a model was trained on has nothing to do with it.
  - **If no hardware materialises, condition 1 stands as written.** An encoder arm that cannot be run at all is recorded as **not executed**, and the descriptor path ships on that ground and no other. It is never recorded as having lost a comparison it did not enter.
