...
(SPECs 0066, 0067, 0068): the composition path is wired end to end on the device,
...
**Lane B has built its pipeline and measured it once** (SPEC 0069, plus the part
...
**Bundled half done, SPEC 0068.** `AssetCorpusStore` reads
...
a shortcut.** SPEC 0069's Scope excludes "the search backend for the 27 unit
