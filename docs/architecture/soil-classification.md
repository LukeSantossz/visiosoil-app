...
| R7 | **Bench-to-field domain gap.** Sieving removes the coarse fraction that most distinguishes Arenosa and air-drying changes colour, so the training subject differs from the deployment subject. This replaced R2 as the dominant unmeasured risk | **Mitigation is OPEN, not absent — this row said "no mitigation available from this dataset" and that is now unsupported.** It assumed two fixed, incomparable worlds. The project owner stated on 2026-08-06 that the product supports both, switchable per case, with more than one field form — one candidate being a sample from 10 cm depth spread on a sheet of paper. A declared capture mode would turn this from an unmeasured gap into a recorded axis, and a paper backing would additionally give a controlled background and a white reference, which matters because soil colour is signal. **Nothing is decided.** Registered as an input in `ml-implementation-map.md` §7. Until it is decided, `setting` is recorded on every row, and **no field-accuracy claim is supportable** |
