...
| Ordinal loss over the classes | Rejected when the list was five, because Siltosa is defined by low sand and sits off the clay axis, so a linear penalty encoded a false geometry. **That reason is gone**: the four the model now emits — Arenosa, Media, Muito Argilosa, Argilosa, spelled and ordered as `ml/config.yaml` declares them — do track increasing clay, though not in that order, since the list is the model's output order and not a granulometric one. The archive still holds five. Still not chosen, now for a different and weaker reason: no arm has been run, so there is no evidence an ordinal penalty helps, and adopting one would add a loss to E0's comparison that no contrast is registered for (SPEC 0044). Reopen it if Siltosa returns to the model, or with a registered arm |
