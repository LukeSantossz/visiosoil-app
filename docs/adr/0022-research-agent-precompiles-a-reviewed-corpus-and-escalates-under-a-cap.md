...
it at lookup time. The app holds that corpus — bundled at install, refreshed by
release — and composes the answer **on the device**, so Tier 1 makes no model call
and no per-record request at all. Live per-record research survives as a third
tier, reached only through deterministic predicates and bounded by an explicit
spend cap.
...
### The app holds the corpus and composes locally; nothing per-record leaves the device

The app ships with a corpus, fetches releases of it, and composes the answer on
the device. There is **no per-record request on Tier 1 at all** — not a coarsened
one, not a region-only one, none.

Two properties follow, and the second is the one that decided it.

The privacy property reaches its strongest form. This design set out to stop
`ProxyResearchService` sending `record.latitude` and `record.longitude`; it ends
by removing the request that carried them. Coarsening location would have been a
mitigation; having nothing to send is not.

And **the composition rule exists once**. Had the proxy answered per record while
the app also composed from a bundled corpus for offline use, the same rule would
live in two languages and have to agree forever. One implementation cannot
disagree with itself. The rule is specified in the architecture document and
pinned by a golden fixture, following the precedent the image-quality criteria
already set in this repository.

`ManagementTipsResult` parses exactly as implemented, so the Drift cache,
`ManagementTipsRepository`, the providers, the Details section and their tests are
untouched — that is where the bulk of the shipped code lives, and none of it is
rebuilt. `ProxyResearchService` is repurposed rather than deleted: its timeout,
retry and typed-failure behaviour carry over to fetching corpus releases and, in
the Tier 2 slice, to the one endpoint that still takes a per-record question.

The `management_tips` cache survives with a different justification. It is no
longer a performance cache, because local composition is cheap enough to repeat.
It is an audit record of what a given record was told and from which corpus
version — a better reason than the one it was built for.

The proxy is therefore a corpus server and a Tier 2 endpoint, which is what fits
inside Cloudflare Workers' 10 ms CPU limit on the free plan. The stable boundary
ADR 0001 drew is what let the transport change this much while the domain
contract did not move at all.
