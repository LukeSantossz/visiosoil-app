...
- CRUX explainers (`.standards/docs/standards/crux_method.md`) apply here as an optional review aid: the `explain-change` skill may render a transient HTML walkthrough of a change to feed R1 and human review. It is never a review layer and never blocks a ship, but its absence is recorded, not silent: if the skill did not run, the reviewer reads the diff directly and the PR notes that the CRUX aid was absent, mirroring the R2 Codex-absent fallback.
...
- `home_screen.dart` has no test file (#120)
